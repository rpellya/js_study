...
for(let i = 0; i < конфеты.length; i++){
   const вкусность = конфеты[i];
   const последняяБукваНаS = вкусность.split('').pop();
   if(последняяБукваНаS === 's'){
      console.log("Нельзя кушать эту шоколадку! На " + вкусность + " у тебя аллегрия");
   }
   else{
      console.log(вкусность + " кушай с удовольстивем!");
   }
}
}
страннаяАллергия();